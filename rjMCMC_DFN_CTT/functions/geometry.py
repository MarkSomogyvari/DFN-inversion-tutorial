...
#Plot stuff
def plot_DFN(DFN,theta):
    plt.clf()
    endpoints = 0
    for i in range(0,len(DFN)):
        if (endpoints ==1):
            plt.scatter(DFN[i,0]-0.5*DFN[i,2]*np.cos(theta[int(DFN[i,3])]),DFN[i,1]-0.5*DFN[i,2]*np.sin(theta[int(DFN[i,3])]))
            plt.scatter(DFN[i,0]+0.5*DFN[i,2]*np.cos(theta[int(DFN[i,3])]),DFN[i,1]+0.5*DFN[i,2]*np.sin(theta[int(DFN[i,3])]))
        plt.plot([DFN[i,0]-0.5*DFN[i,2]*np.cos(theta[int(DFN[i,3])]),DFN[i,0]+0.5*DFN[i,2]*np.cos(theta[int(DFN[i,3])])],[DFN[i,1]-0.5*DFN[i,2]*np.sin(theta[int(DFN[i,3])]),DFN[i,1]+0.5*DFN[i,2]*np.sin(theta[int(DFN[i,3])])],'k')
    plt.show()
    plt.pause(0.001)
    return
...
def AddFracture(DFN,insertion_points,frac_l,dx,theta):
    ...
    if (np.size(insertion_valid) == 0):
        ...
        while(valid == 0 and iterat<max_iter):
            ...
            # Here is where we use the space above/below from insertion_points
            sel_limit_minus = 1
            sel_limit_plus = 0
