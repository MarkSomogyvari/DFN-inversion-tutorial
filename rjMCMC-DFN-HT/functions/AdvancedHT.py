...
def Forward_model(DFN,SR_coord,theta):
    #Discretization
    dx = 1
    ...
    param = [(DFN_Mat,Connect_Mat,SR_coord,P_ini,P_Inj,Inj_id,Prod_id,epsl,Visc,Kapa,DT_time,Dt_T,n_time_P,T_Inj,T_ini,Aper_1,Aper_2,Etha_P,AP,P_Prod,idinj) for idinj in range(0,3)]
    P_new = pool.map(LoopHelp,param)
    pool.close()
    pool.join()
