...
def Forward_model(DFN,SR_coord):
    ...
    K_T = 0.6       # Thermal conductivity of water @ 20 C [J/m.s.K]
    K_r = 3.2       # Thermal conductivity of rock (granite) [J/m.s.K]
    b_size = 10     # Block size for heat loss [m]
    ...
    param = [(DFN_Mat,Connect_Mat,SR_coord,P_ini,P_Inj,Inj_id,Prod_id,epsl,Visc,K_T,K_r,Kapa,b_size,DT_time,Dt_T,n_time_P,T_Inj,T_ini,Aper_1,Aper_2,Etha_P,AP,P_Prod,idinj) for idinj in range(0,Ninj)]
...
def InternalLoop(DFN_Mat,Connect_Mat,SR_coord,P_ini,P_Inj,Inj_id,Prod_id,epsl,Visc,K_T,K_r,Kapa,b_size,DT_time,Dt_T,n_time_P,T_Inj,T_ini,Aper_1,Aper_2,Etha_P,AP,P_Prod,idinj):
    num_Frac = np.shape(DFN_Mat)[0]
...
#                    error = change    #EZT LE KELL ELLENORIZNI + EGYSZERUSITENI!!!
    ...
    for i in range(0,num_Frac):
        ...
        AT[i,i] = AT[i,i]/Gamma_T_tot[i]-Etha_T[i]+h_DFN[i]*DFN_Mat[i,3]
        for j in range(0,5):
            cn = int(Connect_Mat[i,j])
            if (cn >=0):
